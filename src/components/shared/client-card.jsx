...
const ClientCard = ({ review }) => {
  return (
    <div className='flex items-center justify-center gap-6  p-6  border bborder-2 border-primary-800 rounded-2xl bg-white overflow-hidden h-full'>
      <SafeImage src={review?.client_image || review?.image} alt={review?.client_name || review?.name || "client"} width={100} height={100} className='w-1/3 shrink-0  object-contain object-top self-end max-lg:hidden' />
      <div className='text-center lg:pe-6 max-lg:px-6 space-y-4'>
        ...
        <div className='flex items-center justify-between gap-4'>
          <div className='flex items-center justify-center gap-4'>
            <SafeImage src={review?.project_image} alt={review?.project_name || "project"} width={100} height={100} className='size-16 object-cover rounded-full' />
            <div>
